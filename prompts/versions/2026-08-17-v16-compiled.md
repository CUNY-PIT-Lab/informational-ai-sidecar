# Current prompt policy: 2026-08-17-v16
...
- status-faithful wording when a record says a service is on hold, unavailable,
...
- retry instructions for an unnecessary single-source `ASK`, unsupported
  wording, status contradiction, or repeated drafts.
...
A status-contradiction retry requires the affected service's negative status
first. It may mention a separate alternative only when the same record
explicitly presents that alternative as current.
